    ...
    <header style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ color: brutalTheme.muted }} className="hidden md:inline">· portfolio.026</span>
        </div>
        <nav style={{ display: 'flex', gap: 4 }}>
          {['intro', 'about', 'now', 'work', 'play', 'photo', 'contact'].map((n, i) => (
            <a key={n} href={`#br-${n}`} style={{
              padding: '6px 12px',
            ...
            >
              <span className="hidden sm:inline">[{i.toString().padStart(2, '0')}]</span> {n}
            </a>
          ))}
        </nav>
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 14, color: brutalTheme.muted }} className="hidden lg:flex">
          <span>boston</span>
          <span>·</span>
          <span>{time || '—:—'}</span>
          <span>·</span>
